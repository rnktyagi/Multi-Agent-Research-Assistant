...
async def planner_agent_node(state: State):
    ...
    try:
        result = await planner_agent.ainvoke({"messages": [("user", query)]})
...
"""
    ...
    try:
        response = await research_agent.ainvoke({"messages": [("user", objective)]})
...
"""
    ...
    try:
        response = await synthesizer_agent.ainvoke({"messages": [("user", query)]})
...
"""
    ...
    try:
        result = await writer_agent.ainvoke({"messages": [("user", context)]})
...
- Default to approval unless serious problems are detected.
    ...
    try:
        response = await critique_agent.ainvoke({"messages": [("user", query)]})
        result = response.get("structured_response")
    except Exception as e:
